...
func _device_cell(font: Font, at: Vector2, p: int, kind: String,
	...
	# **What he asked for and what he GOT are different questions.** A named
	# pad that is not plugged in falls back to whatever is going, and saying
	# so here is the difference between "it is broken" and "that one is not
	# connected, it took the other".
	var got := "keyboard"
	var col := Color(0.5, 0.75, 0.55)
	if input and int(input.device[p]) >= 0:
		got = kind.to_upper()
		if input.missing[p]:
			got += " *"
			col = Color(1.0, 0.8, 0.4)
	draw_string(font, Vector2(at.x + w + 8.0, at.y + 17.0), got,
