...
func detect() -> void:
	...
	for i in 2:
		device[i] = -1
		missing[i] = false
	# The explicit choices first, so choosing a pad for player two cannot be
	# undone by player one happening to be enumerated first.
	for i in 2:
		...
		for d in pads:
			if not taken.has(d) and ident(int(d)) == pref[i]:
				...
				break
		missing[i] = not got
	# **A choice that cannot be honoured falls back to automatic.**
	#
	# It used to leave the player on the keyboard and nothing else, which is
	# what a saved pad that is no longer plugged in looks like -- so plugging
	# in a DIFFERENT pad did nothing at all and the game "only took the
	# keyboard". A named pad still wins whenever it is actually there; when it
	# is not, anything going is better than nothing, and the menu says so.
	for i in 2:
		if pref[i] == KB or device[i] >= 0:
			continue
		for d in pads:
			if not taken.has(d):
				device[i] = int(d)
